...
run("render=()=>{};nav=()=>{};toast=()=>{};applyMaterials=()=>{};premiumRow=()=>'';openPremium=()=>{state.paywallShown=true};callOptions=()=>{};const materialObserver={observe(){}};function paperLayer(){};const premiumSymbols={};function drivePicture(){return ''};state.premium=false;state.radius=25;state.freeCity=state.city;");
...
run('loadDemoConversation()');assert.equal(run('state.messages[0].read'),false);run("screenGo('chatPreview')");assert.equal(run('state.messages[0].read'),false);run("screenGo('chat')");assert.equal(run('state.messages[0].read'),true);
